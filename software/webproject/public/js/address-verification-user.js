...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const addressInputs = ['user-country', 'user-city', 'user-address'];
...
function verifyAddress() {
    // Get form values - Note: different field names for user form
    ...
    if (countryName.toLowerCase() !== 'greece' && countryName.toLowerCase() !== 'ελλάδα') {
...
/**
 * Transform coordinates from EPSG:4326 (WGS84) to EPSG:900913 (Spherical Mercator)
 */
...
function handleMarkerClick(position, message) {
    ...
    const popup = new OpenLayers.Popup.FramedCloud(
        ...
        position, 
        null,
        "<strong>Τοποθεσία Χρήστη:</strong><br>" + message, 
        null,
